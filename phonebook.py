...
def insertNewContact():
    letterAnswer = input(
        "Should this contact be Simple (S), Friend (F), Professional (P) or Both (B)?")
...
def insertFriendContact():
    ...
    while (not CheckEmail(personalEmail)):
        print("email must contain '.' and '@'")
        personalEmail = input("insert personal Email:")
    contactBook.append(contact.FriendContact(
        name, cellphone, homePhone, personalEmail))
    pass
...
def insertProfessionalContact():
    ...
    while (not CheckEmail(workEmail)):
        print("email must contain '.' and '@'")
        workEmail = input("insert Work Email:")
    contactBook.append(contact.ProfessionalContact(
        name, cellphone, workPhone, workEmail))
    pass
...
def insertProfessionalFriendContact():
    ...
    while (not CheckEmail(workEmail)):
        print("email must contain '.' and '@'")
        workEmail = input("insert Work Email:")
    contactBook.append(contact.ProfessionalFriendContact(
        name, cellphone, homePhone, personalEmail, workPhone, workEmail))
    pass
...
def replaceSimpleContact(ReplacedContact):
    ...
    while not CheckNumber(cellphone):
        if cellphone != 'x':
            print("the number must have only digit")
            cellphone = input(("cellphone: (%s)") %
                              ReplacedContact.getCellphone())
        else:
            cellphone = ""
...
def replaceFriendContact(ReplacedContact):
    ...
    while not CheckNumber(homePhone):
        if homePhone != "x":
            print("the number must have only digit")
            homePhone = input(("home phone: (%s)") %
                              (ReplacedContact.gethomePhone()))
        else:
            homePhone = ""
    personalEmail = input(("personal Email (%s):") %
                          (ReplacedContact.getpersonalEmail()))
    while (not CheckEmail(personalEmail)):
        if personalEmail != "x":
            print("email must contain '.' and '@'")
            personalEmail = input(("personal Email (%s):") %
            ...
            personalEmail = ""
    contact.FriendContact(name, cellphone, homePhone,
                          personalEmail, ReplacedContact)
    pass
...
def replaceProfessional(ReplacedContact):
    ...
    while not CheckNumber(workPhone):
        if workPhone != "x":
            print("the number must have only digit")
            workPhone = input(("Work phone: (%s)") %
                              ReplacedContact.getworkPhone())
        else:
            workPhone = ""
    ...
    while (not CheckEmail(workEmail)):
        if workEmail != "x":
            print("email must contain '.' and '@'")
            workEmail = input(("Work Email (%s):") %
                              ReplacedContact.getWorkEmail())
        else:
            workEmail = ""
    contact.ProfessionalContact(
        name, cellphone, workPhone, workEmail, ReplacedContact)
    pass
...
def replaceProffesionalFriendContact(ReplacedContact):
    ...
    while not CheckNumber(homePhone):
        if homePhone != "x":
            print("the number must have only digit")
            homePhone = input(
                "home phone: (%s)") % ReplacedContact.gethomePhone()
        else:
            homePhone = ""
    personalEmail = input(
        "personal Email (%s):") % ReplacedContact.getpersonalEmail()
    while (not CheckEmail(personalEmail)):
        if personalEmail != "x":
            print("email must contain '.' and '@'")
            personalEmail = input(
    ...
    while not CheckNumber(workPhone):
        if workPhone != "x":
            print("the number must have only digit")
            workPhone = input(
                "Work phone: (%s)") % ReplacedContact.getworkPhone()
        else:
            workPhone = ""
    workEmail = input("Work Email (%s):") % ReplacedContact.getWorkEmail()
    while (not CheckEmail(workEmail)):
        if workEmail != "x":
            print("email must contain '.' and '@'")
            workEmail = input(
                "Work Email (%s):") % ReplacedContact.getWorkEmail()
        else:
            workEmail = ""
    contact.ProfessionalFriendContact(
        name, cellphone, homePhone, personalEmail, workPhone, workEmail, ReplacedContact)
    pass
    ...
    for i in range(len(contactBook)):
        for j in range(len(contactBook) - 1 - i):
            if contactBook[j].__lt__(contactBook[j + 1]):
                contactBook[j], contactBook[j +
                                            1] = contactBook[j + 1], contactBook[j]
